...
function start_time(val) {
    ...
    setInterval(() => {
        ...
        if (time_diff <= 0) {
            ...
            console.log(chalk.italic.bold.rgb(224, 56, 123)("\n\t\t-----------Thank you-----------\n"));
            process.exit();
        }
        const min = Math.floor((time_diff % (3600 * 24)) / 3600);
        const sec = Math.floor(time_diff % 60);
        console.log(chalk.bold.rgb(23, 224, 15)(`\t\t${min.toString().padStart(2, "0")}:${sec
            .toString()
            .padStart(2, "0")}\n `));
    }, 1000);
}
